#!/bin/bash
clear

###############################
#
# Parametros:
#  - Lista Dominios y URL
#
#  Tareas:
#  - Se debera generar la estructura de directorio pedida con 1 solo comando con las tecnicas enseñadas en clases
#  - Generar los archivos de logs requeridos.
#
###############################
LISTA=$1

sudo mkdir -p /temp/{Error/{cliente,servidor},ok}

LOG_FILE="/var/log/status_url.log"
LOG_OK="/temp/ok/dominio.log"


ANT_IFS=$IFS
IFS=$'\n'

for LINEA in `cat $LISTA |  grep -v ^#`
do
#---- Dentro del bucle ----#
  # Obtener el código de estado HTTP
  URL=$(echo $LINEA |awk '{print $2}')
  STATUS_CODE=$(curl -LI -o /dev/null -w '%{http_code}\n' -s "$URL")

  # Fecha y hora actual en formato yyyymmdd_hhmmss
  TIMESTAMP=$(date +"%Y%m%d_%H%M%S")

 # Registrar en el archivo /var/log/status_url.log
  echo "$TIMESTAMP - Code:$STATUS_CODE - URL:$URL" |sudo tee -a  "$LOG_FILE"
    
  DOMINIO=$(echo $LINEA |awk '{print $1}')
  if [[ $STATUS_CODE == 200 ]]; then
    echo "$TIMESTAMP - Code:$STATUS_CODE - URL:$URL" |sudo tee -a  /temp/ok/$DOMINIO.log
  fi

  if [[  $STATUS_CODE -ge 400  &&  $STATUS_CODE -lt 500  ]]; then
    echo "$TIMESTAMP - Code:$STATUS_CODE - URL:$URL" |sudo tee -a  /temp/Error/cliente/$DOMINIO.log
  fi

  if [[ $STATUS_CODE -ge 500 ]]; then
    echo "$TIMESTAMP - Code:$STATUS_CODE - URL:$URL" |sudo tee -a  /temp/Error/servidor/$DOMINIO.log
  fi

done


#-------------------------#

IFS=$ANT_IFS

sudo tree /temp/
